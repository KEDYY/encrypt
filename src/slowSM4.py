...
最后修改时间：2014年8月13日
"""
__author__   = 'Kisa.Y.Chang <yifei0727@gmail.com>' 
__date__     = '2014/8/13' 
__version__  = '0.1.0' 
...
def switch_box( int_src):
    u"""
       `int_src`  待置换数  1字节数据
    """
    return chr(SBox.get(ord(int_src)))
...
def XOR(data1, data2, force_right=False):
    ...
    if force_right:
        ...
        for i in range(len(data1)):
            rdata += chr(ord(data1 [i]) ^ ord(data2[i]))   #  += 较快于 ''.join
    else:
        if len(data1) != len(data2):
            raise ValueError("Param1's Length != Param2's Lehgth")
        rdata = ""
        for i in range(len(data1)):
...
from binascii import unhexlify as unhexs, hexlify  as hexs
class SM4():
    ECB = 0x00
    CBC = 0x01
    ...
    def __init__(self, KEY, mode=ECB, iv=None, pad=None, padmode='PAD_NORMAL'):
        ...
        self.rk = self._create_key(unhexs(KEY))
        self.rk_enc = self.rk
    ...
    def decrypt(self, src_data):
        ...
        """
        return self._crypt(src_data, self.rk_dec)
